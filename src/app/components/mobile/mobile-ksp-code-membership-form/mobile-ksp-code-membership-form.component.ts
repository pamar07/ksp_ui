...
@Component({
  selector: 'app-mobile-ksp-code-membership-form',
  templateUrl: './mobile-ksp-code-membership-form.component.html',
  styleUrls: ['./mobile-ksp-code-membership-form.component.css'],
  providers: [DatePipe]
})
export class MobileKspCodeMembershipFormComponent implements OnInit {
          ...
          if (this.payment_mode == "Online Payment via Credit/Debit Card and Net Banking") {
            ...
            this._apiService.kspCodeInsta(instaMojo_info).subscribe(ret1 => {
              ...
              if ($ret1 == true) {
                ...
                if (this.plan == "Skill Builder Box for Rs.1200" || this.plan == "Play To Learn Box for Rs.999" || this.plan == "Festive Fun Box for Rs.999" || this.plan == 'Busy Box for Rs.999' || this.plan == 'Summer Playdate Box') {
                ...
                let dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: msg });
                dialogRef.afterClosed().subscribe(result => {
                  if (this.plan == "Festive Fun Box for Rs.999") {
                    window.location.href = 'https://imjo.in/BTzVcr';
            ...
            let dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'We have received your order. Your box will be delivered within 15 working days from the date of recieved of payment.' });
            dialogRef.afterClosed().subscribe(result => {
  ...
  validateForm() {
    if (this.mother_name == null || (this.mother_name).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Name cannot be empty!' });
      return false;
    }
    if (this.mother_mobile == null || (this.mother_mobile).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Mobile No cannot be empty!' });
      return false;
    }
    if (!this._validateService.validateMobile(this.mother_mobile)) {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Invalid Mobile No!' });
      return false;
    }
    if (this.mother_email == null || (this.mother_email).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Email cannot be empty!' });
      return false;
    }

    if (this.building_name == null || (this.building_name).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Building Name cannot be empty!' });
      return false;
    }
    if (this.block_no == null || (this.block_no).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Landmark cannot be empty!' });
      return false;
    }
    if (this.road == null || (this.road).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Area cannot be empty!' });
      return false;
    }
    if (this.landmark == null || (this.landmark).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'City cannot be empty!' });
      return false;
    }
    if (this.area == null || (this.area).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'State cannot be empty!' });
      return false;
    }
    if (this.pincode == null || (this.pincode).toString().trim() == '') {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Pincode cannot be empty!' });
      return false;
    }
    if (this.accept == false) {
      const dialogRef = this.dialog.open(MobileKspMembershipDialog, { data: 'Please accept Terms & Conditions and Privacy Policy of the website' });
      return false;
    }
    return true;
  }
...
export class MobileKspMembershipDialog {
