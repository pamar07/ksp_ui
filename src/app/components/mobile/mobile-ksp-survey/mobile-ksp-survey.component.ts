...
@Component({
  selector: 'app-mobile-ksp-survey',
  templateUrl: './mobile-ksp-survey.component.html',
  styleUrls: ['./mobile-ksp-survey.component.css'],
  providers: [Cookie]
})
export class MobileKspSurveyComponent implements OnInit {
  ...
  sendForgotOtp(data){
    this.api.sendEmailOTP(data.email).subscribe(ret=>{
      let $ret = ret.ret;
      if($ret.code == 1){
        let dialogRef = this.dialog.open(MobileDialog,{data:'An OTP has been sent to your email id'});
        this.authTab = 'forgot-password-otp';
      }
      else{
        let dialogRef = this.dialog.open(MobileDialog,{data:'Sorry, this email address is not registered in our platform'});
      }
    },
    err => {
      let dialogRef = this.dialog.open(MobileDialog,{data:'Some problem has occured.<br/>Please try after some time.'});
    });
  }
        ...
        if(this.forgotOtp > 0){
          this.api.verifyEmailOTP(this.forgotPassForm.controls['email'].value,this.forgotOtp).subscribe(ret=>{
            ...
            if($ret.code == 1){
              ...
              let dialogRef = this.dialog.open(MobileDialog,{data:'OTP is incorrect.<br/>Please enter the correct OTP.'});
            }
          },
          err => {
            let dialogRef = this.dialog.open(MobileDialog,{data:'Some problem has occured.<br/>Please try after some time.'});
          ...
          let dialogRef = this.dialog.open(MobileDialog,{data:'Incorrect format of OTP.'});
        }
      }
      else{
        let dialogRef = this.dialog.open(MobileDialog,{data:'OTP must be a number.'});
      }
    }
    else{
      let dialogRef = this.dialog.open(MobileDialog,{data:'Please enter the OTP.'});
    }
  }
  ...
  changePasswordByEmail(data){
    if(data.password == data.confirm_password){
        this.api.changePasswordByEmail(this.forgotPassForm.controls['email'].value,data.password).subscribe(ret=>{
          let $ret = ret.ret;
          if($ret.code == 1){
            let dialogRef = this.dialog.open(MobileDialog,{data:'Password changed successfully.<br/>Please login with new password.'});
            this.authTab = 'login';
          }
          else{
            let dialogRef = this.dialog.open(MobileDialog,{data:'Password cannot be changed.<br/>Some problem has occured.'});
          }
        },
        err => {
          let dialogRef = this.dialog.open(MobileDialog,{data:'Some problem has occured.<br/>Please try after some time.'});
      ...
      let dialogRef = this.dialog.open(MobileDialog,{data:'Password and confirm password is not same.<br/>Please check.'});
    }
  }
  ...
  submitRegForm(data) {
    ...
    this.sendOtp();
    this.saveVoterInfo();
    let dialogRef = this.dialog.open(MobileDialog,{data:'An OTP has been sent to your email address and mobile number'});
    this.showOtp = 1;
  }
  ...
  updateUserVote() {
    ...
    this.api.updateUserVote(data).subscribe(ret=>{
      ...
      if($error.code == 1) {
        this._route.navigateByUrl('/Awards2017');
        let dialogRef = this.dialog.open(MobileDialog,{data:$error.msg});
      } else {
        this.currentPage = 'thankyou';
        this.saveFrontVote();
      }
