...
@Component({
  selector: 'app-mobile-event-individual',
  templateUrl: './mobile-event-individual.component.html',
  styleUrls: ['./mobile-event-individual.component.css'],
  providers: [DatePipe]
})
export class MobileEventIndividualComponent implements OnInit {
    ...
    this._route.params.subscribe(params => {
      ...
      this._apiService.getIndividualEvent(this.id).subscribe(ret=>{
        ...
        if($ret.code == 1){
          ...
          try{
            this.eventIndividual.content = this.eventIndividual.content.split("<img ").join("<img class=\"img-responsive\"");
  ...
  participate(){
    let dialogRef = this.dialog.open(MobileParticipateConfirmDialog,{data:'You are going to participate in this event'});
        ...
        if(this._apiService.loggedIn()){
          this._apiService.participateEvent(this.eventIndividual.id).subscribe(ret=>{
            ...
            if($ret.code == 1){
              ...
              let dialogRef = this.dialog.open(MobileParticipatedDialog,{data:'Thank you for participating this event...'});
            }else{
            }
          },err=>{
          });
...
export class MobileParticipateConfirmDialog {
...
export class MobileParticipatedDialog {
 constructor(@Inject(MD_DIALOG_DATA) public data: any) { }
}
