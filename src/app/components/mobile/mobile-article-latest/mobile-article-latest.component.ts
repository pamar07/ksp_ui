...
@Component({
  selector: 'app-mobile-article-latest',
  templateUrl: './mobile-article-latest.component.html',
  styleUrls: ['./mobile-article-latest.component.css'],
  providers: [DatePipe]
})
export class MobileArticleLatestComponent implements OnInit {
  ...
  updateArticles() {
    this.loading = true;
    this._apiService.getArticlesRecent(1, this.noOfItems, this.selectedCity, this.selectedAge).subscribe(ret => {
      let $ret = ret.ret;
      if ($ret.code == 1) {
        this.articles = $ret.data.recent;
  ...
  getNewPage(pageNo) {
    if (pageNo < 1) {
      ...
      this.loading = true;
      this._apiService.getArticlesRecent(pageNo, this.noOfItems, this.selectedCity, this.selectedAge).subscribe(ret => {
        let $ret = ret.ret;
        if ($ret.code == 1) {
          this.articles = $ret.data.recent;
        }
