...
@Component({
  selector: 'app-web-camp-all',
  templateUrl: './web-camp-all.component.html',
  styleUrls: ['./web-camp-all.component.css'],
  providers: [DatePipe]
})
export class WebCampAllComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._apiService.getCities().subscribe(ret=>{
      ...
      if($ret.code == 1){
        ...
        for(let i=0; i<this.cityList.length; i++){
          if(this.selectedCity.some(x=>x===this.cityList[i].city)){
            this.cityList[i].checked = true;
          }
        }
    ...
    this._apiService.getEditorCamp(this.selectedCity,this.selectedAge).subscribe(ret=>{
      let $ret = ret.ret;
      if($ret.code == 1){
        this.editorEvents = $ret.data;
      } else {
        this.editorEvents = [];
      }
    },err=>{
