...
@Component({
  selector: 'app-web-brand-dashboard',
  templateUrl: './web-brand-dashboard.component.html',
  styleUrls: ['./web-brand-dashboard.component.css']
})
export class WebBrandDashboardComponent implements OnInit {
  ...
  logo1: any;
  brandlogo: any;
  brandForm: FormData = new FormData();
  ...
  updatePassword(data){
    ...
    this._apiService.updatePassword(data.password).subscribe(ret => {
      const $ret = ret.ret;
      if ($ret.code == 1){
        const dialogRef = this.dialog.open(BrandDashboardDialog, {data: 'Password changed successfully'});
      }
    }, err => {
    });
  }
  ...
  sendAdvertiseMail() {
    const dialogRef = this.dialog.open(BrandDashboardDialog, {data: 'Thank you! We have received your request. Our team will shortly get back to you.'});
  ...
  onSubmit() {
    console.log(this.registerForm.value.companyName);
    this.showErrormessage = 0;
    // this.submitted = true;
    // for (let k = 0; k < this.arr.length ; k++) {
    //   this.catId.push(this.arr[k].id);
    // }
    ...
    if (this.brandlogo) {
      const data = {
        category_id: this.catId.toString(),
      };
      this.brandForm.append('category_id', this.catId.toString());
      this.brandForm.append('logo', this.brandlogo, this.brandlogo.name);
      console.log(this.brandForm);
      this._apiService.nominationSubmitByBrand(this.brandForm).subscribe(ret => {
        if (ret['ret'].code === 1) {
          this.nomination = false;
          this.thankYou = true;
        }
...
export class BrandDashboardDialog {
