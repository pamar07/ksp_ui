...
@Component({
  selector: 'app-web-event-submit',
  templateUrl: './web-event-submit.component.html',
  styleUrls: ['./web-event-submit.component.css'],
  providers: [DatePipe],
})
export class WebEventSubmitComponent implements OnInit {
  ...
  validate(){
    ...
    try{
      ...
      //let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Invalid Date Format...'});
    }
    ...
    if(this.event_title.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event title...'});
    }
    else if(formatted_start_date.toString().trim()=="")
    {
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event start date and time...'});
    }
    else if(formatted_end_date.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event end date and time...'});
    }
    /*else if(this.event_slot.toString().trim() == ''){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event slot...'});
    }
    else if(formatted_participate_date.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event participation date and time...'});
    }*/
    else if(this.venue_name.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event venue name...'});
    }
    else if(this.venue_city.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event venue city...'});
    }
    else if(this.google_map.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event venue address...'});
    }
    // else if(this.event_lat.toString().trim()=="" || this.event_lng.toString().trim()==""){
    //   let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please fetch the latitude and longitude by clicking on the Fetch Location button...'});
    // }
    else if(this.content.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide the event content...'});
    }
    else if(checkedAgeGroups == ''){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please select atleast one age group based on this event...'});
    }
    else if(checkedCategories == ''){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please select atleast one interest category based on this event...'});
    }
    else{
      this.submitEvent();
    }
  ...
  submitEvent(){
    ...
    this._apiService.submitEvent(event).subscribe(ret=>{
      ...
      let $ret = ret.ret;
      if($ret.code == 1){
        let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Your event has been submitted successfully.<br/>Events will be shown publicly after moderation.'});
    ...
    if(fileList.length > 0) {
      ...
      if(file.size<=(1024*1024*5)){
        if(file.type == 'image/jpg' || file.type == 'image/jpeg' || file.type == 'image/png'){
          ...
          let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please upload .jpg or .jpeg or .png type file only...'});
        }
      }
      else{
        let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'File size should not be more than 1 Mb...'});
      }
    }
  }
  ...
  getLatitudeLongitude() {
    if(!this.google_map){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide a valid address'});
    }
    else if(this.google_map.toString().trim()==""){
      let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Please provide a valid address'});
    }
  ...
  /*setLatitudeLongitude(latlng) {
      ...
      //let dialogRef = this.dialog.open(WebEventSubmitDialog,{data:'Unable to fetch the location! <br/> Please ensure that you have entered a valid address.'});
...
export class WebEventSubmitDialog {
