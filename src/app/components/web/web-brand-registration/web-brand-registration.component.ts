...
@Component({
  selector: 'app-web-brand-registration',
  templateUrl: './web-brand-registration.component.html',
  styleUrls: ['./web-brand-registration.component.css']
})
export class WebBrandRegistrationComponent implements OnInit {
    ...
    if(data.email != '' && data.phone != '' && this.regForm.controls.email.valid && this.regForm.controls.phone.valid) {
      this.api.sendRegistrationOTP(data.email, (data.code + data.phone), 'KSPSMS').subscribe(ret=>{
        ...
        if($ret.code == 1){
          ...
          this.otpStatus = true;
          // let dialogRef = this.dialog.open(WebBrandRegistrationDialog,{data:'An OTP has been sent to your email address and mobile number'});
        }else{
          this.sendOtpBtn = 'Send OTP';
          let dialogRef = this.dialog.open(WebBrandRegistrationDialog,{data: 'There is some network error, Please try after sometime.'});
        }
      },err=>{
      });
    } else {
      this.sendOtpBtn = 'Send OTP';
      let dialogRef = this.dialog.open(WebBrandRegistrationDialog,{data:'Please enter valid email and phone number.'});
    }
  }
      ...
      setTimeout(()=>{
        ...
        this.api.registerBrand(brandInfo).subscribe(ret=>{
          ...
          if($ret.code == 1){
            ...
            let dialogRef = this.dialog.open(WebBrandRegistrationDialog,{data:'Thank you for registering with us.'});
            ...
            let dialogRef = this.dialog.open(WebBrandRegistrationDialog,{data:'We are facing some problem.<br/>Please try again.'});
          }
        },
        err => {
        });
  ...
  getLatitudeLongitude() {
    if(!this.google_map_address){
      let dialogRef = this.dialog.open(WebBrandRegistrationDialog,{data:'Please provide a valid address'});
    }
...
export class WebBrandRegistrationDialog {
