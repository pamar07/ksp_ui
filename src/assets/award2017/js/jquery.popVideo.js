$(function () {
    ...
    PopVideo.prototype.init = function (element, options) {
        ...
        var control = " <svg class=\"popvideo_svg_sprite\" display=\"none\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\">\n" +
            ...
            "            </div>\n" +
            "        </div>";
        var tpl = '<div class="popvideo-wrapper" id="' + popid + '">' + '<a href="javascript:void(0)" class="extra-popvideo-close">&times;</a>' +
            '<div class="popvideo">' +
            ...
            control +
            '</div> ' + '<a href="" download="" class="download-video-btn"> <i class="fa fa-download"></i> </a>'+ '</div>';

        $('body').append(tpl);
        this.$wrapper = $('#' + popid);
        this.$video = this.$wrapper.find('video');
        this.$anchorTag = this.$wrapper.find('a');
        ...
        this.$wrapper.find('.popvideo-close').on('click.popvideo.close', function (e) {
            e.preventDefault();
            self.close();

        });
        this.$wrapper.find('.extra-popvideo-close').on('click.popvideo.close', function (e) {
            e.preventDefault();
            self.close();
...
// 声音控制
    ...
    PopVideo.prototype.setVideo = function (video) {
        ...
        this.$anchorTag.attr('href', video);
    };
